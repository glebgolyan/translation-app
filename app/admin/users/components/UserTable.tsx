...
export function UserTable({ users, isLoading }: UserTableProps) {
    ...
    if (isLoading) return <Center py={12}><Spinner color="brand.500" /></Center>;
            ...
            <Tbody bg={bg}>
                {users.map((user: User) => (
                    <Tr key={user.id} _hover={bg}>
                        ...
                        <Td><Text fontSize="13px" fontFamily="mono" color="gray.600">{user.email}</Text></Td>
                        <Td><Text fontSize="13px" color="gray.500">{user.phone || '—'}</Text></Td>
                        <Td>
                            <Select
                                size="xs"
                                value={user.role}
