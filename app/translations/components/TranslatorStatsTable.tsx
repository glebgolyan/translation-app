'use client';
import {
    Box, Table, Thead, Tbody, Tr, Th, Td, HStack,
    Input, Button, Icon, useToast, Flex, Text,
    useColorModeValue, Center, Spinner, VStack,
...
export function TranslatorStatsTable() {
    ...
    const actualDaysInMonth = new Date(monthDate.getFullYear(), monthDate.getMonth() + 1, 0).getDate();
    ...
    const handleCellClick = (statId: string | null, currentValue: number, translatorId: string, day: number) => {
    ...
    return (
        <VStack spacing={8} align="stretch">
            ...
            <Flex gap={4} align="center">
                <Text fontFamily="Syne" fontWeight="600" fontSize="14px">
                ...
                <Text fontSize="12px" color={thColor} ml="auto">
                    Click cells to edit
                </Text>
            </Flex>
            ...
            <Box bg={bg} borderRadius="8px" border="1px solid" borderColor={borderColor} overflow="hidden">
                <Box overflowX="auto">
                    <Table variant="simple" size="sm">
                        <Thead bg={theadBg}>
                            <Tr>
                                <Th width="140px" color={thColor} fontSize="12px" px={3}>
                                    Translator
                                </Th>
                        ...
                        <Tbody>
                            {stats.map((row: any) => (
                                <Tr key={row.translatorId} _hover={{ bg: hoverBg }} transition="background 0.1s">
                                    <Td
                                        width="140px"
                                        fontSize="13px"
                                        fontWeight="500"
                                            ...
                                            >
                                                {isEditing ? (
                                                    ...
                                                    <Text
                                                        ...
                                                        onClick={() => handleCellClick(statId, wordCount, row.translatorId, day)}
                                                    >
                                                        {wordCount}
                                                    </Text>
                                                )}
            ...
            {weekTotals.map(week => (
                <Box key={week.number}>
                    <Text fontFamily="Syne" fontWeight="700" fontSize="14px" color={weekHeaderColor} mb={4}>
                    ...
                    <Box bg={bg} borderRadius="8px" border="1px solid" borderColor={borderColor} overflow="hidden">
                        <Table variant="simple" size="sm">
                            <Thead bg={theadBg}>
                                <Tr>
                                    <Th color={thColor} fontSize="12px" px={4} py={3}>
                                        Translator
                                    </Th>
                                    <Th color={thColor} fontSize="12px" px={4} py={3} textAlign="right">
                                        Week Total
                                    </Th>
                                </Tr>
                            </Thead>
                            <Tbody>
                                {stats.map((row: any) => {
                                    ...
                                    return (
                                        <Tr
                                            ...
                                            transition="background 0.1s"
                                        >
                                            <Td fontSize="13px" fontWeight="500" color={tdColor} px={4} py={3}>
                                                {row.translatorName}
                                            </Td>
                                            <Td
                                                fontSize="14px"
                                ...
                                {/* Week subtotal */}
                                <Tr bg={summaryBg}>
                                    <Td fontSize="13px" fontWeight="700" color={summaryTextColor} px={4} py={3}>
                                        Week Total
                                    </Td>
                                    <Td
                                        fontSize="15px"
                                        ...
                                        fontFamily="mono"
                                    >
                                        {stats.reduce((sum: number, row: any) => sum + (week.translators[row.translatorId]?.total || 0), 0).toLocaleString()}
                                    </Td>
                                </Tr>
                            </Tbody>
                        </Table>
                        ...
                        <Tbody>
                            ...
                            <Tr bg={summaryBg}>
                                ...
                                <Td
                                    ...
                                    fontFamily="mono"
                                >
                                    {stats.reduce((sum: number, row: any) => sum + (grandTotals[row.translatorId]?.total || 0), 0).toLocaleString()}
                                </Td>
                            </Tr>
                        </Tbody>
                    </Table>
