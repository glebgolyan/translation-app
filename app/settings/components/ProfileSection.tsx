'use client';
import {
    Box, Text, Grid, FormControl, FormLabel, Input,
    Button, Icon, useToast, VStack, HStack, Avatar,
        ...
        <Box bg={bg} borderRadius="12px" border="1px solid" borderColor={borderColor} overflow="hidden">
            ...
            <Box p={6}>
                ...
                <HStack spacing={4} mb={8}>
                    <Avatar size="lg" name={user.name} bg="brand.100" color="brand.700" />
                    <Box>
                        <Text fontFamily="Syne" fontWeight="700" fontSize="18px" color={textColor}>
                            {user.name}
                        </Text>
                        <HStack spacing={2} mt={1}>
                            <Badge
                                px={2} py={0.5} borderRadius="4px"
                            ...
                            <Text fontSize="13px" color={labelColor}>{user.email}</Text>
                        </HStack>
                    </Box>
                </HStack>
                ...
                <Box as="form" onSubmit={handleSubmit(onSubmit)}>
                    <Grid templateColumns="repeat(2, 1fr)" gap={4} mb={6}>
                        <FormControl>
                            <FormLabel fontSize="13px" color={labelColor}>{t('auth.name')}</FormLabel>
                            <Input {...register('name')} size="sm" />
                        </FormControl>

                        <FormControl>
                            <FormLabel fontSize="13px" color={labelColor}>{t('auth.email')}</FormLabel>
                            <Input {...register('email')} type="email" size="sm" />
                        </FormControl>

                        <FormControl>
                            <FormLabel fontSize="13px" color={labelColor}>{t('auth.phone')}</FormLabel>
