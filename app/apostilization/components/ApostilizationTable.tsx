...
export function ApostilizationTable({ month, search, onEdit }: ApostilizationTableProps) {
    ...
    if (isLoading) {
        return <Center py={12}><Spinner color="brand.500" /></Center>;
    }
        ...
        <Box bg={bg} borderRadius="8px" border="1px solid" borderColor={borderColor} overflow="hidden">
            <Box overflowX="auto">
                <Table variant="simple" size="sm">
                    <Thead bg={theadBg}>
                        <Tr>
                            <Th color={thColor} fontSize="12px" px={4}>{t('apostilization.dateOfTaking')}</Th>
                            <Th color={thColor} fontSize="12px" px={4}>{t('apostilization.clientName')}</Th>
                            <Th color={thColor} fontSize="12px" px={4}>{t('apostilization.documentType')}</Th>
                            <Th color={thColor} fontSize="12px" px={4}>{t('apostilization.contact')}</Th>
                            <Th color={thColor} fontSize="12px" px={4} textAlign="right">{t('apostilization.sum')}</Th>
                            <Th color={thColor} fontSize="12px" px={4} textAlign="right">{t('apostilization.remaining')}</Th>
                            <Th color={thColor} fontSize="12px" px={4}>{t('apostilization.status')}</Th>
                            <Th color={thColor} fontSize="12px" px={4}>{t('apostilization.actions')}</Th>
                        </Tr>
                    </Thead>
                    <Tbody>
                        {items.map((item: any) => (
                            <Tr key={item.id} _hover={{ bg: hoverBg }}>
                                <Td fontSize="13px" color={tdColor} px={4} py={3}>
                                    {new Date(item.dateOfTaking).toLocaleDateString('uk-UA')}
                                </Td>
                                <Td fontSize="13px" color={tdColor} px={4} py={3}>{item.clientName}</Td>
                                <Td fontSize="13px" color={tdColor} px={4} py={3}>{item.documentType}</Td>
                                <Td fontSize="13px" color={tdColor} px={4} py={3}>{item.contact}</Td>
                                <Td fontSize="13px" color={tdColor} px={4} py={3} textAlign="right" fontFamily="mono" fontWeight="600">
                                    ₴{item.sum.toLocaleString()}
                                </Td>
                                <Td fontSize="13px" color={item.remainingAmount > 0 ? 'orange.500' : 'green.500'} px={4} py={3} textAlign="right" fontFamily="mono" fontWeight="600">
                                    ₴{item.remainingAmount.toLocaleString()}
                                </Td>
                                <Td fontSize="13px" px={4} py={3}>
                                    <Select
                                        size="xs"
                                        value={item.status}
